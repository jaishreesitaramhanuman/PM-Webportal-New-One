...
export type GenerateBriefingNotesInput = z.infer<typeof GenerateBriefingNotesInputSchema>;
...
export type GenerateBriefingNotesOutput = z.infer<typeof GenerateBriefingNotesOutputSchema>;

export async function generateBriefingNotes(
  input: GenerateBriefingNotesInput
): Promise<GenerateBriefingNotesOutput> {
...
const generateBriefingNotesFlow = ai.defineFlow(
  ...
  async input => {
    ...
    return {
      ...output!,
      progress: 'Generated a briefing note summarizing key insights from the consolidated report.',
    };
  }
);
